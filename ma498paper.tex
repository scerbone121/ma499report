...
%Note that in the interest of brevity, some of the more elementary definitions and concepts will not be included in this paper. For additional information regarding Equivalence Relations, Binary Relations, [insert more if necessary], please ask your friendly neighborhood mathematician for assistance. Thank you.
...
As is defined in [book] fundamental group of a surface, centered at a point $x_0$, is the collection of all loops that you can possibly draw which start and end on $x_0$. %TODO: citation
...
But what \textit{is} the fundamental group of the circle? In Algebraic Topology, by [author], we have the following theorem: %TODO: citation
\begin{theorem}
...
%Now, imagine a woman named Brenda, who asks you to hang these doughnuts from the ceiling fan. You want to hang them so that they $(1)$ don't fall and $(2)$ don't get crushed. 
...
A crucial property with Möbius transforms is that if you take four distinct, nonzero points on a circle, or line, since circles and lines are one in the same here on $\hat\C$, you can transform them into any other combination of four distinct, nonzero points using Möbius transforms. Below is a theorem, courtesy of a handout written by Rich Schwartz: %TODO: citation
...
% \section{Notable Mistakes}

% I realized that after I had already written working code and generated some images that I had misunderstood the purpose of the assignment. Recall how each of the sides of the universal covering are {distinctly labeled} and \underline{associated with its own cross ratio}. The fact that each of the sides must have its own cross ratio had slipped my mind and I had used one cross ratio for \underline{all} of the sides.

% The problem here is our points plotted using a cross ratio with an imaginary component no longer lie on the circle. We specifically want to choose $a$, $b,$ and $c$ such that
% \[(1+ai)(1+bi)(1+ci)=1,\]
% which would ensure that all the plotted points will lie on the boundary of a circle. Since I used a slider input to determine the imaginary cross ratio component, which again is the \textit{same} for all the sides, my points have quite literally gone off the rails. I had
% \begin{align*}
% (1+ai)(1+ai)(1+ai)=(1+ai)^3=(1-3a^2)+i(3a-a^3),
% %\implies a=0,\, a=\frac{\sqrt3}{2}\pm i\frac{3}{2}
% \end{align*}
% which is only equal to $1$ if we have $a=0$, which was my test case to ensure the points were correctly plotted, and $a=(\sqrt3\pm3i)/2$. This of course meant that the complex points I have plotted were not on a circle, nor was the resulting image simply connected.  Instead, I created a slightly-rough-looking Cayley graph, pictured in the following section.

% Despite not being the goal of the project, I decided to include these images because I think they look nice. Additionally, making mistakes also provides an opportunity to learn more, and to even find interesting results that you were not looking for. 

% %\includegraphics{CR_loglog30}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{CR_0}
    \caption{$CR=1$}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{CR_loglog4}
    \caption{$CR=1+i\log(\log(4))$}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{CR_loglog9}
    \caption{$CR=1+i\log(\log(9))$}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{CR_loglog30}
